...
//     const user = await User.find({org_id : req.user.org_id});
//     if(!user){
...
const updateUser = asyncHandler(async(req,res)=>{
    ...
    if (user_role) {
        ...
        if(role.org_id.toString() !== req.user.org_id.toString()){
            return res.status(403).json({
...
const deleteMultipleUsers = asyncHandler(async (req, res) => {
    ...
    const org_users = await User.find({
        _id : { $in : user_ids },
        org_id : req.user.org_id
    });

    if(org_users.length !== user_ids.length){
        return res.status(403).json({
            success : false,
            message : "Some Users Do Not Belong To Your Organization"
        });
    }
    ...
    return res.status(200).json({
        ...
        deleted_count: deleted_users.deletedCount
    });
